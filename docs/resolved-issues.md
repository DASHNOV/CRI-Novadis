...
## [2026-09-07] `__EFMigrationsHistory` désynchronisé du schéma — l'API n'aurait pas redémarré

- **Symptôme** : aucun, en production — c'est le point important. Défaut **latent**, détecté lors d'un contrôle avant déploiement (phase 0 du plan de remédiation), et qui ne se serait manifesté qu'au redémarrage suivant : `ADD COLUMN "SavedSignature"` sur une colonne déjà présente → PostgreSQL `42701 duplicate_column` → `Program.cs:407` relance l'exception → l'API refuse de démarrer, et `restart: always` la fait boucler.
- **Cause** : les trois migrations du dépôt étaient dans trois états différents. `20260527092951_InitialCreate` appliquée et enregistrée ; `20260624000000_AddUserSavedSignature` **appliquée en base mais absente de l'historique** ; `20260730121446_RemoveCriServicePriority` ni appliquée ni enregistrée (colonne `Priority` et index `IX_CRIForms_Priority` toujours présents). L'indice de la cause racine est l'identifiant `20260624000000`, qui se termine par six zéros : `dotnet ef migrations add` produit un horodatage réel (`20260527092951`, `20260730121446`). Cette migration a donc été **écrite à la main** pour documenter après coup une colonne ajoutée directement en base, sans que `__EFMigrationsHistory` soit mis à jour dans le même geste. EF ne voyant qu'`InitialCreate`, il aurait rejoué la migration 2 au démarrage suivant.
- **Correctif** : sauvegarde préalable (restauration validée le jour même, cf. `scripts/README-backup.md`), puis enregistrement de la seule migration déjà appliquée — `INSERT INTO "__EFMigrationsHistory" VALUES ('20260624000000_AddUserSavedSignature', '8.0.11')`. `RemoveCriServicePriority` a été délibérément **laissée en attente** : son DDL n'ayant jamais été exécuté, c'est à EF de le jouer au déploiement suivant, plutôt qu'à du SQL écrit à la main qui risquerait de diverger de ce qu'EF attend. L'existence de l'index `IX_CRIForms_Priority` a été vérifiée au préalable, la migration commençant par un `DropIndex`.
- **Prévention** : **toute modification de schéma passée directement en base doit être enregistrée dans `__EFMigrationsHistory` dans le même geste**, et une migration écrite à la main pour rattraper l'existant doit l'être aussi. Sans cela, la dette reste invisible jusqu'au déploiement suivant, où elle se paie en indisponibilité. Contrôle systématique avant tout déploiement touchant au backend : comparer `SELECT "MigrationId" FROM "__EFMigrationsHistory"` au contenu de `Data/Migrations/` — un écart doit être résolu **avant** de déployer, jamais pendant. Un identifiant de migration se terminant par des zéros signale une migration non générée par la CLI, donc à vérifier en priorité.
