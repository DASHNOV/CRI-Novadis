...
- **Cause** : quatre défauts cumulés.
  1. `catch (_) {}` autour de `uploadPhotos` dans `SyncService` **et** dans les deux contrôleurs de formulaire (soumission en ligne) : l'échec était avalé, puis `syncStatus: 'synced'` écrit.
  2. L'envoi héritait des délais Dio globaux (10 s d'envoi), taillés pour du JSON : quelques Mo de photos en 4G terrain les dépassaient régulièrement — la cause d'échec la plus probable, masquée par (1).
  3. Côté serveur, un fichier > 10 Mo ou d'un type inattendu était ignoré (`continue`) avec une réponse **200** : l'app le croyait envoyé.
  4. Aucune déduplication : rejouer l'envoi (nécessaire pour corriger 1) aurait dupliqué les photos déjà reçues.
- **Correctif** : suppression des `catch (_) {}` — un échec de photos laisse le CRI « pending », rejoué par `SyncService` (recul exponentiel existant) ; délais dédiés à l'envoi (3 min / 1 min) ; `ApiException` pour que le motif d'un refus s'affiche sur la carte. Serveur : validation de tous les fichiers **avant** écriture, 400 motivé (nom du fichier, taille ou type) ; photo déjà enregistrée pour le CRI (même nom, même taille) ignorée → rejeu idempotent. Tests : `test/services/sync_service_test.dart` (6 cas, dont 3 échouent sur l'ancien code) et `PhotoUploadTests` (3 échouent sur l'ancien code).
- **Prévention** : **un `catch (_) {}` sur une écriture réseau est une perte de données en attente** — un échec doit soit remonter, soit laisser l'élément dans la file de rejeu. Tout envoi rejouable doit être idempotent côté serveur. Et jamais de 200 sur une requête partiellement ignorée. Les APK antérieurs gardent l'ancien comportement (catch côté client) : les photos déjà « perdues » restent sur les appareils concernés.
